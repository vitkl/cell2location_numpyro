...
class LocationModelLinearDependentWMultiExperimentPyro(PyroLocModel):
    ...
    * **combs_per_spot** prior tells the model how much co-location signal to expect between the programmes / cell types.
    ...
    def __init__(
        ...
        self.cell2sample_df = pd.get_dummies(sample_id)
        # convert to np.ndarray
        self.cell2sample_mat = self.cell2sample_df.values
        self.n_exper = self.cell2sample_mat.shape[1]
        # assign extra data to dictionary with input data
        # self.extra_data = {'cell2sample': self.cell2sample_mat.astype(self.data_type)}
        self.cell2sample = jnp.array(self.cell2sample_mat.astype(self.data_type))
    ...
    def model(self, x_data):
        ...
        self.gene_E = pyro.sample('gene_E', dist.Exponential(jnp.ones([self.n_exper, self.n_var]) * self.phi_hyp[0, 0]))

        # =====================Expected expression ======================= #
        # expected expression
        self.mu_biol = jnp.dot(self.spot_factors, self.gene_factors.T) * self.gene_level.T \
                       + jnp.dot(self.cell2sample, self.gene_add) + self.spot_add

        self.theta = jnp.dot(self.cell2sample,
                             jnp.ones([1, 1]) / (self.gene_E * self.gene_E))
    ...
    def compute_expected(self):
        ...
        self.alpha = np.dot(self.cell2sample_mat,
